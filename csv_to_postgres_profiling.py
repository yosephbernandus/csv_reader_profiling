#!/usr/bin/env python3
"""
Comprehensive profiling of CSV parsing and PostgreSQL insertion
Compares Python CSV vs Rust CSV parser performance with PostgreSQL operations
"""
...
try:
    ...
    print(
        "WARNING: psycopg2 not installed. Please install with: pip install psycopg2-binary"
    )
    POSTGRES_AVAILABLE = False


class CSVtoPostgresProfiler:
    def __init__(self, csv_file, db_params, batch_size=5000, table_name="transactions"):
    ...
    def check_postgres_connection(self):
        """Verify PostgreSQL connection and create test table"""
        if not POSTGRES_AVAILABLE:
            print("PostgreSQL profiling not available without psycopg2")
            return False
    ...
    def clear_test_table(self):
        """Clear the test table for a fresh start"""
        if not POSTGRES_AVAILABLE:
            return
        ...
        try:
            ...
            cursor.close()
            conn.close()
            print(f"Table {self.table_name} cleared for testing")
        except Exception as e:
            print(f"Error clearing test table: {e}")
...
class CSVtoPostgresProfiler:
    ...
    def python_csv_to_postgres(self):
        """
        Read CSV with Python's CSV module and insert into PostgreSQL
        """
        if not POSTGRES_AVAILABLE:
            return 0, 0
        ...
        with open(self.csv_file, "r", newline="") as f:
            ...
            for row in reader:
                ...
                if len(batch) >= self.batch_size:
                    execute_values(
                        cursor,
                        f"INSERT INTO {self.table_name} (id, amount) VALUES %s",
                        batch,
                        template="(%s, %s)",
                    )
                    conn.commit()
    ...
    def pandas_csv_to_postgres(self):
        """
        ...
        try:
            ...
            print("Pandas or SQLAlchemy not installed, skipping pandas test")
            return 0, 0

        # Create SQLAlchemy engine from psycopg2 connection parameters
        db_url = f"postgresql://{self.db_params['user']}:{self.db_params['password']}@{self.db_params['host']}:{self.db_params.get('port', 5432)}/{self.db_params['dbname']}"
        ...
        # Number of operations is approximately rows/batch_size
        db_operations = (rows_processed + self.batch_size - 1) // self.batch_size

        return rows_processed, db_operations

    def rust_csv_to_postgres(self):
        """
        Read CSV with Rust parser and insert into PostgreSQL
        """
        if not POSTGRES_AVAILABLE or not RUST_AVAILABLE:
            return 0, 0
    ...
    def run_benchmarks(self):
        """Run all benchmarks"""
        if not POSTGRES_AVAILABLE:
            print("ERROR: PostgreSQL benchmarks require psycopg2 to be installed")
            print("Install with: pip install psycopg2-binary")
            return

        print("Starting benchmarks...")

        # Python CSV to PostgreSQL
        self.profile_function("Python CSV to PostgreSQL", self.python_csv_to_postgres)

        # Pandas to PostgreSQL (if available)
        try:
            import pandas
            from sqlalchemy import create_engine

            self.profile_function("Pandas to PostgreSQL", self.pandas_csv_to_postgres)
        except ImportError:
            print("\nSkipping Pandas benchmark (pandas or sqlalchemy not installed)")

        # Rust CSV to PostgreSQL
        if RUST_AVAILABLE:
            self.profile_function("Rust CSV to PostgreSQL", self.rust_csv_to_postgres)
    ...
    def print_summary(self):
        ...
        for name, result in self.results.items():
            if name == "Python CSV to PostgreSQL":
        ...
        for name, result in self.results.items():
            ...
            if name != "Python CSV to PostgreSQL" and baseline:
    ...
    def generate_visualization(self):
        ...
        try:
            ...
            fig.suptitle("CSV to PostgreSQL Performance Comparison", fontsize=16)
            ...
            plt.savefig("csv_to_postgres_performance.png")
            print("\nVisualization saved to 'csv_to_postgres_performance.png'")
...
def main():
    parser = argparse.ArgumentParser(
        description="Profile CSV to PostgreSQL performance"
    )
    ...
    parser.add_argument(
        "--table", type=str, default="transactions", help="PostgreSQL table name to use"
    )
    # PostgreSQL connection parameters
    parser.add_argument("--host", type=str, default="localhost", help="PostgreSQL host")
    parser.add_argument("--port", type=int, default=5432, help="PostgreSQL port")
    parser.add_argument(
        "--dbname", type=str, required=True, help="PostgreSQL database name"
    )
    ...
    # Run profiling
    profiler = CSVtoPostgresProfiler(csv_file, db_params, args.batch_size, args.table)
